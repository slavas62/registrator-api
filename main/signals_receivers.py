...
def post_change(*args, **kwargs):
    ...
    if created:
        for w in wow:
            new = ModelDef(name='%ss_for_%s' % (w, instance.name), owner=instance.owner, resource=True,
                           hidden=True, verbose_name=wow[w][0], verbose_name_plural=wow[w][1])
            new.save()
            ForeignKeyDefinition(name='object', model_def_id=new.contenttype_ptr_id,
                                 to_id=instance.contenttype_ptr_id, related_name='%ss' % w,
                                 verbose_name=instance.verbose_name).save()
            FilePathFieldDefinition(name='file', model_def_id=new.contenttype_ptr_id,
